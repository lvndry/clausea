...
| Variable                                      | Build | Description                                                            |
| --------------------------------------------- | ----- | ---------------------------------------------------------------------- |
| `NEXT_PUBLIC_PADDLE_CLIENT_TOKEN`             | Yes\* | Optional local-dev fallback; production checkout loads token from API  |
| `NEXT_PUBLIC_PADDLE_ENVIRONMENT`              | Yes\* | Optional local-dev fallback; must match backend `PADDLE_ENVIRONMENT`   |
...
| "Unable to open checkout" / checkout overlay fails | Set `PADDLE_CLIENT_TOKEN` on the **API** service (Paddle Dashboard → Developer tools → Authentication → Client-side tokens). Must match `PADDLE_ENVIRONMENT`. Verify `GET /subscriptions/checkout-config` returns a non-empty `client_token`. Redeploy API, then frontend.                                                                                |
